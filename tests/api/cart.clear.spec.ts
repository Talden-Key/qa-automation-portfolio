...
async function postWithBackoff(request: any, url: string, retries = 3) {
  ...
  for (
    let attempt = 0;
    attempt < retries && response.status() === 429;
    attempt++
  ) {
...
test.describe("Shopify Ajax Cart API - Clear Cart", () => {
  test("POST /cart/clear.js empties the cart", async ({ request }) => {
    ...
    expect(clearRes.status(), "Expected /cart/clear.js to succed (200).").toBe(
      200
    );
    ...
    expect(
      cart.item_count,
      "Expected cart.item_count to be 0 after clearing."
    ).toBe(0);

    expect(
      cart.item.length,
      "Expected cart.items array to be empty after clearing."
    ).toBe(0);
  });
});
